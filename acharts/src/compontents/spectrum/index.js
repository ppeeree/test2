...
import { getClosestNumber } from '../../tools/YBLine/ybCommonTools.js'
let chartPaddingTop = 40
let chartPaddingBottom = 70
let chartGriBwn = 30
// 此组件为频域分析，游标工具：单游标、双游标、边频游标、倍频游标
...
export class SpectrumChart {
  constructor(echart, echartData, datazoomEvent, { theme, isSetY, isSetX, isAddNote, isDataDown, isStacked, isShuang,
    ...
    this.selectedPointList = [] // 高亮点集合
    this.clickedMarklineInfo = {} // 点击选中的markline信息

    this.Toolbox = new Toolbox(this.myChart, this.themeType, {
      isFullData: this.isFullData,
      isYB: true,
      isZoomY: false,
      isSetX: typeof (isSetX) == 'undefined' ? true : isSetX,
      ...
      YBList: {
        ...
        isBei: typeof (isBei) == 'undefined' ? true : isBei,
        isBian: typeof (isBian) == 'undefined' ? true : isBian,
        isGap: typeof (isGap) == 'undefined' ? false : isGap,
        isPeak: true,
      },
      YBEvent: this.YBOperate.bind(this)
    }, this.hotUpdate.bind(this), this.changeStackedType.bind(this), /* this.toolboxClickedEvent.bind(this) */)
  ...
  // 数据更新响应
  unitChangeRespondData(echartData) {
    this.echartData = { ...this.echartData, ...echartData }
    let options = this.myChart.getOption()
    const { dimensions } = echartData
    ...
    this.myChart.setOption({
      series: [...newSeries],
      xAxis: Array.from(xAxis, i => ({
        ...i, name: dimensions
        [0]
      })),
      yAxis: Array.from(yAxis, i => ({
        ...i, name: dimensions
        [1]
      })),
    })
  ...
  // 工具条组件中调用或者修改实例属性
  hotUpdate(data, dataZoom, options) {
    if (data) {
      for (let key in data) {
        this[key] = data[key]
 ...
 */
  initData(lineDataSourse) {
    ...
    data.forEach(i => {
      ...
      /*  if (i.xInterval) {
         i.source = i.source.map((ii, index) => {
           return [accMul(index, i.xInterval), ii]
         })
       } */
    })
...
*/
  ...
  changeStackedType(type, theme) {
    ...
    const { xAxisType, yAxisType, data, dimensions } = this.echartData
    let relationOp = {
      axisPointer: {
        link: {
          xAxisIndex: "all",
    ...
    //  let maxXais = data.map(i => i.xAxisMaxHz)
    let xAxisBase = {
      ...xAxisStyle,
      ...
      axisLabel: {
        width: 100,
        textStyle: {
          ...xAxisStyle.axisLabel.textStyle
        }
      },
    };
    let yAxisBase = {
  ...
  initOperate() {
    ...
    this.myChart.on('legendselectchanged', params => {
      // this.Toolbox.toolboxClickedEvent('重置', null, this.myChart)
      const { selected } = params
      let selectedLegend = []
      Object.keys(selected).forEach(i => {
        if (selected[i]) {
          selectedLegend.push(i)
        }
      })
      ...
      setTimeout(() => {
        if (this.dataZoomDataList.length) {
          const { start, end, startValue, endValue } = this.dataZoomDataList[this.dataZoomDataList.length - 1][0]
          this.myChart.dispatchAction({
            type: 'dataZoom',
            start,
            end,
    ...
    this.myChart.on('click', param => {
      ...
      clickTimer = setTimeout(() => {
        ...
        if (param.componentType == "markLine" && param.seriesName !== 'markline_level') {
          let { id } = param.data
          that.clickedMarklineInfo = param.data
          that.myYB.changeLastYBLine(id)
          that.myYB.lastYBType = id.split('_')[0]  //  "beipinGroup0_1"
        }
        if (param.componentType == 'series') {
          let { transform } = param.event.target
          that.handleTooltipBlock(param, transform.slice(-2))
        }
        // 清除定时器
        clickTimer = null;
  ...
  */
  YBOperate(title, value) {
    ...
    } else if (title == 'changeLineNum') {
      this.myYB.changeYBLineNum(this.rightClickedYBLineId, value)
    }
  }
  ...
  // 增加倍频/边频标记线
  addMarkLine(lineData, lineType) {
    let markLineName = lineType == 'SF' ? this.clickedMarklineInfo.name + '_' + lineType + '_' + this.clickedMarklineInfo.value : lineType
    let options = this.myChart.getOption()
    let { id, yAxisIndex, data, markLine, markPoint, name, color } = options.series.find(i => i.name == this.selectedLegend[0])
    let others = markLine.data.filter(i => i.name !== markLineName)
    let othersPoint = markPoint.data.filter(i => i.name !== markLineName)
    let points = []
    let lineArr = lineData.map((item, index) => {
      let { closeIndex, closeValue } = getClosestNumber(data, item)
      points.push({
        name: markLineName,
        coord: closeValue,
        itemStyle: {
        ...
        dataType: lineType == 'SF' ? (index > (lineData.length / 2 - 1) ? index - 2 : index - 3) : (index == 0 ? lineType + '_1X' : index + 1),
        emphasis: {
          label: {
            show: false
          }
      ...
      return {
        ...
        lineStyle: {
          color: '#333',
          width: 1,
          type: lineType == 'SF' ? 'dashed' : lineType == 'RF' ? 'dotted' : 'solid',
        },
        dataType: index == 0 ? lineType + '_1X' : index + 1,
        label: {
          show: lineType != 'SF',
          color: '#333',
          formatter: (param) => {
  ...
  removeMarkLine(lineType) {
    let options = this.myChart.getOption()
    let { id, markLine, markPoint } = options.series.find(i => i.name == this.selectedLegend[0])
    let others = markLine.data.filter(i => !i.name.includes(lineType))
    let othersPoint = markPoint.data.filter(i => !i.name.includes(lineType))
    this.myChart.setOption({
      series: [{
        id,
        markLine: {
  ...
  removeAllFRMarkLine(typeList) {
    let options = this.myChart.getOption()
    let needResetMaklineSeries = []
    options.series.forEach(item => {
      let { id, markLine, markPoint } = item
      if (markLine.data.length) {
        item.markLine.data = markLine.data.filter(i => {
          return typeList.every(element => !i.name.includes(element))
        })
        item.markPoint.data = markPoint.data.filter(i => {
          return typeList.every(element => !i.name.includes(element))
        })
        needResetMaklineSeries.push(item)
      } else {
        needResetMaklineSeries.push(item)
      }
    })
