...
export class RPMWave {
  constructor(echart, dataSource, { theme, isStacked, isSetY, isAddNote, isDataDown } = {} /* clickEvent, alertMessage */) {
    this.myChart = echart
      ...
      let options = {
        ...
        tooltip: {
          ...
          formatter: function (params) {
            let str = ''
            if (params.length) {
              str += `<p style='text-align:center;margin-bottom:5px;border-bottom:1px solid #ccc'>${params[0].axisValue}</p>`
              params.forEach(item => {
                str +=
  ...
  getSeries(data, stackedType, yAxisName, lineArr) {
    let arr = []
    if (data.length) {
