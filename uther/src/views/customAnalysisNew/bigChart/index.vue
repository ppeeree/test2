<template>
  <div class="bigChartContent" id="bigChartContent">
    ...
    <i
      v-if="['Waterfall', 'RCA', 'PCA', 'WCA', 'OA', 'DA'].indexOf(chartType) < 0"
    ...
    <template>
      <diagnosticRecord
        ...
        :imgUrl="imgUrlDialog"
        :key="chartType"
        :chartType="chartENZHList[chartType]"
...
import { chartENZHList } from '../components/toolsComponent/tools.js'
export default {
  components: {
    emptyChart,
    Splitter,
  ...
  created() {
    ...
    const { chartType, type, acqPointerInfoList, treeDataAndFilterP } = decodedObject
  ...
  methods: {
    ...
    addRecord() {
      // 设定规则：只能给一条曲线增加分析记录
      if (this.$refs.bigChartUnit.myChartInstance.selectedLegend.length != 1) {
        return this.$message.warning('请选择一条曲线进行分析！')
      }
      let reader = new FileReader()
      let data = this.$refs.bigChartUnit.myChartInstance.imageBase64Data(true)
      reader.readAsDataURL(data)
      let recordChartInfo = this.$refs.bigChartUnit.chartData.data.find(
        item => item.name == this.$refs.bigChartUnit.myChartInstance.selectedLegend[0]
      ).others
      reader.onload = e => {
