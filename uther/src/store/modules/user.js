...
const user = {
  state: {
    ...
    userDeptTree: getStore({ name: 'userDeptTree' }) || [],
  },
  actions: {
    //根据用户名登录
    ...
    GetDeptTree({ commit }, param) {
      return new Promise((resolve) => {
        getEnitiyTree({ userId: param }).then(res => {
          commit('SET_DEPTTREE', res.data.data)
          resolve()
        })
      })
    },
  ...
  mutations: {
    ...
    SET_DEPTTREE: (state, userDeptTree) => {
      state.userDeptTree = userDeptTree
      setStore({ name: 'userDeptTree', content: state.userDeptTree })
    },
    SET_DEPTTREE_NULL: (state) => {
      state.userDeptTree = []
      setStore({ name: 'userDeptTree', content: state.userDeptTree })
    },
  }
