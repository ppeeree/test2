 ...
 * @param {String} filename 文件名称
 */
export const downloadTxt = (fileArrayBuffer, filename) => {
  let data = new Blob([fileArrayBuffer], { type: 'text/plain;charset=utf-8' })
  if (typeof window.chrome !== 'undefined') {
    // Chrome
    let link = document.createElement('a')
    link.href = window.URL.createObjectURL(data)
...
export const generateUniqueHexColors = (numColors) => {
  let colors = new Set()
